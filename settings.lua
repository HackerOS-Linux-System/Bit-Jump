-- settings.lua: Rozbudowane ustawienia
...
settingsOptions = {
    {name = "Difficulty", values = {"Easy", "Normal", "Hard"}, selected = 2},
    {name = "Fullscreen", values = {"Off", "On"}, selected = 1},
    {name = "Sound Volume", values = {"0%", "25%", "50%", "75%", "100%"}, selected = 5},
    {name = "Music", values = {"Off", "On"}, selected = 2},
    {name = "Resolution", values = {"800x600", "1024x768", "1280x720", "1920x1080"}, selected = 1},
    {name = "Graphics Quality", values = {"Low", "Medium", "High"}, selected = 2},
                                        ...
                                        if setting.selected < 1 then setting.selected = #setting.values end
                                            ...
                                            if setting.name == "Difficulty" then
                                                ...
                                                elseif setting.name == "Fullscreen" then
                                                    ...
                                                    elseif setting.name == "Sound Volume" then
                                                        settings.soundVolume = (setting.selected - 1) * 0.25
                                                        elseif setting.name == "Music" then
                                                            settings.musicOn = (setting.selected == 2)

...
use super::trading::*;
use super::events::*;
use super::save_load::{SaveGame, load_game};
...
const MAP_WIDTH: usize = 100; // Adjust as per your map size
...
fn setup_game(
    ...
    // Initialize events
    events.events = vec![
        resources::Event { description: "Bandit Ambush".to_string(), effect: EventEffect::SpawnEnemies(EnemyType::Bandit, 4), duration: 30.0 },
        resources::Event { description: "Treasure Discovery".to_string(), effect: EventEffect::DropItems(Item { name: "Gold".to_string(), value: 50, item_type: ItemType::Currency }, 2), duration: 10.0 },
        resources::Event { description: "Sudden Storm".to_string(), effect: EventEffect::WeatherChange(Weather::Storm), duration: 60.0 },
        resources::Event { description: "Arcane Surge".to_string(), effect: EventEffect::SpawnEnemies(EnemyType::DarkMage, 3), duration: 20.0 },
        resources::Event { description: "Peasant Uprising".to_string(), effect: EventEffect::FactionBoost(4, 50), duration: 20.0 },
    ];

    // Spawn player
    let player_class = class_selection.map(|c| c.0.clone()).unwrap_or(SocialClass::Peasant);
    let (health, mana, player_skills) = match player_class {
            ...
            transform: Transform::from_xyz(0.0, 0.0, 1.0),
                    sprite: Sprite {
                        color: match player_class {
                            SocialClass::Knight => Color::srgb(0.0, 0.0, 1.0),
                    SocialClass::Noble => Color::srgb(0.5, 0.0, 0.5),
                    SocialClass::Priest => Color::srgb(1.0, 1.0, 1.0),
                    SocialClass::Townsfolk => Color::srgb(0.0, 0.5, 0.5),
                    SocialClass::Peasant => Color::srgb(0.5, 0.3, 0.2),
                    SocialClass::Bandit => Color::srgb(0.3, 0.1, 0.1),
                    SocialClass::Mage => Color::srgb(0.2, 0.2, 0.8),
                        },
    ...
    for i in 0..300 {
        ...
        if let Some(id) = quest {
            quests.active.push(Quest {
                ...
                faction_id,
                required_class: Some(social_class),
            });
        }
        commands.spawn((
            SpriteBundle {
                transform: Transform::from_xyz(x, y, 1.0),
                        sprite: Sprite {
                            color: match social_class {
                                SocialClass::Knight => Color::srgb(0.0, 0.0, 0.8),
                        SocialClass::Noble => Color::srgb(0.6, 0.0, 0.6),
                        SocialClass::Priest => Color::srgb(0.9, 0.9, 0.9),
                        SocialClass::Townsfolk => Color::srgb(0.0, 0.6, 0.6),
                        SocialClass::Peasant => Color::srgb(0.6, 0.4, 0.3),
                        SocialClass::Bandit => Color::srgb(0.4, 0.1, 0.1),
                        SocialClass::Mage => Color::srgb(0.3, 0.3, 0.9),
                            },
                            custom_size: Some(Vec2::new(TILE_SIZE, TILE_SIZE)),
                        ..default()
                        },
                ...
                transform: Transform::from_xyz(x, y, 1.0),
                        sprite: Sprite {
                            color: match enemy_type {
                                EnemyType::RogueKnight => Color::srgb(0.3, 0.3, 0.7),
                        EnemyType::DarkMage => Color::srgb(0.2, 0.2, 0.6),
    ...
    // Spawn shops
    let world_map = commands.get_resource::<WorldMap>().unwrap();
    for region in world_map.regions.iter() {
...
fn player_movement(
    ...
    if let Ok((mut transform, player)) = player_query.get_single_mut() {
        ...
        if direction.length() > 0.0 {
            ...
            .filter(|s| if let SkillEffect::Buff(Stat::Speed, _, _) = s.effect { true } else { false })
                ...
                if player_trans.translation.distance(npc_trans.translation) < TILE_SIZE * 2.0 {
                    ...
                    if let Some(quest_id) = npc.quest_id {
                        ...
                        if !quest.completed && quest.required_class.as_ref().map_or(true, |c| *c == player.social_class) {
                            ...
                            // Accept quest logic here if needed
                        }
                    }
...
fn update_fog_of_war(
    ...
    if let Ok(player_trans) = player_query.get_single() {
        for (tile_trans, mut tile) in tile_query.iter_mut() {
            let distance = player_trans.translation.distance(tile_trans.translation.truncate());
...
fn restock_shops(
    mut shop_query: Query<(&Transform, &mut Shop)>,
                 time: Res<Time>,
                 world_map: Res<WorldMap>,
) {
    for (transform, mut shop) in shop_query.iter_mut() {
        shop.restock_timer -= time.delta_seconds();
        if shop.restock_timer <= 0.0 {
            let shop_x = (transform.translation.x / TILE_SIZE) as usize;
            let shop_y = (transform.translation.y / TILE_SIZE) as usize;
            let region = world_map.regions.iter().find(|r| {
                let (rx, ry) = r.center;
                let dx = shop_x as i32 - rx as i32;
                let dy = shop_y as i32 - ry as i32;
                dx*dx + dy*dy <= (r.radius as i32).pow(2)
            });
            shop.inventory = if let Some(r) = region {
                if r.tile_type == TileType::Castle {
                    vec![
                        Item { name: "Greatsword".to_string(), value: 1, item_type: ItemType::Weapon },
                        Item { name: "Plate Armor".to_string(), value: 1, item_type: ItemType::Armor },
                        Item { name: "Mana Crystal".to_string(), value: 2, item_type: ItemType::Consumable },
